...
export function AdminJobDetailPage() {
  ...
  return (
    <div>
      <AdminPageHeader title={isLoading ? 'Loading…' : data?.title ?? 'Job Detail'} subtitle={data ? `${data.status} · ${data.tradeType}` : undefined} actions={<Link to="/admin/jobs"><Button variant="secondary" size="sm">Back to Jobs</Button></Link>} />
